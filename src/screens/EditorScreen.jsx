...
function voiceMoveAllPreviewText(template, data, extra = {}) {
 const vars = {
 ...data,
 user: 'Akashsuu',
 tag: 'Akashsuu#0000',
 id: '123456789012345678',
 mention: '@Akashsuu',
 sourceChannel: 'General Voice',
 sourceChannelId: data.sourceChannelId || '111222333444555666',
 targetChannel: 'Staff Voice',
 targetChannelId: data.targetChannelId || '777888999000111222',
 movedCount: '5',
 failedCount: '0',
 totalCount: '5',
 server: 'My Server',
 channel: 'general',
 command: `!${data.command || 'vmoveall'}`,
 reason: data.reason || 'No reason provided',
 error: 'Missing permissions',
 ...extra,
 };
...
const ANTINUKE_FEATURES = [
 ...
 ['antiInviteRole', 'Anti Invite Role'], ['autoRecovery', 'Auto Recovery'],
];

function antinukePreviewText(template, data, extra = {}) {
 const vars = {
 ...data,
 server: 'My Server',
 serverId: '123456789012345678',
 status: data.enabledByDefault ? (data.enabledText || 'Enabled') : (data.disabledText || 'Disabled'),
 action: 'channel delete',
 executor: 'BadActor#0000',
 executorId: '999888777666555444',
 executorMention: '@BadActor',
...
function DiscordPreviewVoiceMoveAll({ node }) {
 const { botInfo } = useProject();
 const now = new Date();
 const time = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
 const d = node?.data || {};
 const botName = botInfo?.username || 'Bot';
 const text = voiceMoveAllPreviewText(
 d.successMessage || 'Moved **{movedCount}** users from **{sourceChannel}** to **{targetChannel}**.\nReason: {reason}',
 d
 );
 ...
 <div className="dc-avatar" style={{ display: botInfo?.avatarURL ? 'none' : 'flex', background: d.embedColor || '#6366F1' }}>VA</div>
 <div className="dc-msg-body">
 <div className="dc-msg-hdr">
 <span className="dc-bot-name">{botName}</span>
 <span className="dc-bot-badge">BOT</span>
 ...
 data={{
 ...d,
 embedColor: d.embedColor || '#6366F1',
 embedTitle: d.embedTitle || 'Voice Move All',
 embedFooter: voiceMoveAllPreviewText(d.embedFooter || '{movedCount}/{totalCount} users moved', d),
 }}
 text={text}
 />
 )}
...
function DiscordPreviewAntiNuke({ node }) {
 const { botInfo } = useProject();
 const now = new Date();
 const time = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
 const d = node?.data || {};
 const botName = botInfo?.username || 'Bot';
 const tick = d.tickText || '✓';
 const cross = d.crossText || '✕';
 const featureLines = ANTINUKE_FEATURES.map(([key, label]) => `${label}: ${d[key] === false ? cross : tick}`);
 const firstBlock = featureLines.slice(0, 21).join('\n');
 const secondBlock = featureLines.slice(21).join('\n');
 const panelDescription = [
 antinukePreviewText(d.panelDescription || 'Prevent harmful activities with active monitoring.', d),
 '',
 antinukePreviewText(d.statusLine || 'Current Status: {status}', d),
 ].join('\n');

 const Button = ({ children, color }) => (
 <button type="button" style={{ background: color, color: '#fff', border: 0, borderRadius: 6, padding: '8px 14px', fontWeight: 800, fontSize: 12 }}>
 {children}
 </button>
 );
 ...
 <div className="dc-avatar" style={{ display: botInfo?.avatarURL ? 'none' : 'flex', background: d.embedColor || '#3B82F6' }}>AN</div>
 <div className="dc-msg-body">
 <div className="dc-msg-hdr">
 <span className="dc-bot-name">{botName}</span>
 <span className="dc-bot-badge">BOT</span>
 ...
 <DiscordEmbed
 data={{ ...d, embedColor: d.embedColor || '#3B82F6', embedTitle: antinukePreviewText(d.panelTitle || 'Configure Antinuke', d), embedFooter: antinukePreviewText(d.featuresFooter || 'Powered by Kiodium Development', d) }}
 text={panelDescription}
 />
 <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 10, marginBottom: 14 }}>
 <Button color="#058F52">{d.enableButtonLabel || 'Enable'}</Button>
 <Button color="#DA373C">{d.disableButtonLabel || 'Disable'}</Button>
 <Button color="#5865F2">{d.featuresButtonLabel || 'Features'}</Button>
 </div>
 <DiscordEmbed
 data={{ ...d, embedColor: d.embedColor || '#3B82F6', embedTitle: antinukePreviewText(d.featuresTitle || 'Antinuke Features Enabled', d), embedFooter: antinukePreviewText(d.featuresFooter || 'Powered by Kiodium Development', d) }}
 text={`${firstBlock}\n\n${secondBlock}`}
 />
 </div>
 </div>
...
function NPanel({ selectedNode, setNodes }) {
 ...
 const isVoiceMoveAllPreview = selectedNode.type === 'moderation_vmoveall';
 const isAntiNukePreview = selectedNode.type === 'moderation_antinuke';
 ...
 ) : isVoiceMoveAllPreview ? (
 <DiscordPreviewVoiceMoveAll node={selectedNode} />
 ) : isAntiNukePreview ? (
 <DiscordPreviewAntiNuke node={selectedNode} />
 ) : isMusicPlayPreview ? (
